...
@Autonomous(name = "CloseAutoRED", group = ".")
    ...
    public void start() {
        Command autoSeq = new SequentialCommandGroup(
                new InstantCommand(() -> {
                    ...
                    firstShotPose = new Pose2d(targetPos, p.heading);
                ...
                new InstantCommand(() -> {
                    ...
                    double heading = p.heading.toDouble();
                    double dx = FORWARD_DIST * Math.cos(heading);
                    double dy = FORWARD_DIST * Math.sin(heading);
                ...
                new InstantCommand(() -> {
                    if (firstShotPose != null) {
                        Pose2d p = mD.localizer.getPose();
                        Actions.runBlocking(
                                mD.actionBuilder(p)
                                        .strafeToLinearHeading(
                                                new Vector2d(
                                                        firstShotPose.position.x,
                                                        firstShotPose.position.y
                                                ),
                                                firstShotPose.heading
                                        )
                                        .build()
                        );
                    }
