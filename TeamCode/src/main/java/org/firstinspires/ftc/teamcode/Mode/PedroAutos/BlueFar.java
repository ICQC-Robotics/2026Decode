...
/**
 * Blue-alliance-only split of ZayansFarAuto -- fixed to BLUE, no alliance prompt, no mirroring
 * helpers (every position/heading/turret-angle constant below is already blue-relative).
 * Behavior is otherwise identical. See RedFar for the mirrored counterpart.
 */
...
public class BlueFar extends CommandOpMode {
    ...
    public void initialize() {
        ...
        negabot.vision.setAlliance(true);
        ...
        while (!isStarted() && !isStopRequested()) {
            ...
            telemetry.addLine("=== BLUE FAR — PUSH TO START ===");
        ...
        return new SequentialCommandGroup(
                ...
                new ConditionalCommand(
                        shotPrep(f, new VisionGuidedZoneApproach(
                                f, negabot.vision, true, zoneEndpoints, Math.toRadians(0),
                                () -> zoneResult, ZONE_RESAMPLE_MS, ZONE_COMMIT_DIST_IN,
                                new Pose(SHOOT_POS_X, SHOOT_POS_Y, Math.toRadians(0)), SLOW_ZONE_X_IN, SLOW_ZONE_POWER),
    ...
    private int getZone() {
        int z = negabot.vision.getScannedZone(true);
        double[] d = negabot.vision.getLastDensity(true);
    ...
    void buildPaths(Follower f) {
        ...
        toHP = f.pathBuilder()
                .addPath(new BezierLine(new Pose(pushedStart.getX(), pushedStart.getY()), new Pose(8.5, 8)))
                .setLinearHeadingInterpolation(pushedStart.getHeading(), Math.toRadians(0))
                .addPath(new BezierLine(new Pose(8.5, 8), new Pose(11.5, 8)))
                .setLinearHeadingInterpolation(Math.toRadians(0), Math.toRadians(0))
                .addPath(new BezierLine(new Pose(11.5, 8), new Pose(8.5, 8)))
                .setLinearHeadingInterpolation(Math.toRadians(0), Math.toRadians(0))
                .build();

        toShootFromHP = f.pathBuilder()
                .addPath(new BezierLine(new Pose(8.5, 8), new Pose(SHOOT_POS_X, SHOOT_POS_Y)))
                .setLinearHeadingInterpolation(Math.toRadians(0), Math.toRadians(0))
                .build();
        ...
        to3rdRow = f.pathBuilder()
                ...
                .setHeadingInterpolation(easedReversedTangent(thirdRowCurve, Math.toRadians(0), CURVE_HEADING_EASE_T))
                .build();

        toShootFrom3rdRow = f.pathBuilder()
                .addPath(new BezierLine(new Pose(20, 36), new Pose(SHOOT_POS_X, SHOOT_POS_Y)))
                .setLinearHeadingInterpolation(Math.toRadians(THIRD_ROW_EXIT_HEADING_DEG), Math.toRadians(0))
                .build();
        ...
        zoneEndpoints[1] = new Pose(8.5, 8);
        zoneEndpoints[2] = new Pose(8.5, 14);
        zoneEndpoints[3] = new Pose(8.5, 20);
        zoneEndpoints[4] = new Pose(8.5, 26);
        zoneEndpoints[5] = new Pose(8.5, 32);
        ...
        toSweep = f.pathBuilder()
                .addPath(sweepCurve)
                .setHeadingInterpolation(easedReversedTangent(sweepCurve, Math.toRadians(0), CURVE_HEADING_EASE_T))
                .build();
        toShootFromSweep = f.pathBuilder()
                .addPath(new BezierLine(new Pose(SWEEP_END_X, SWEEP_END_Y), new Pose(SHOOT_POS_X, SHOOT_POS_Y)))
                .setLinearHeadingInterpolation(Math.toRadians(SWEEP_EXIT_HEADING_DEG), Math.toRadians(0))
                .build();
    }
