 ...
 */
@Autonomous(name = "no first spike CloseAuto24", group = "Close")
public class NoFirstSpikeClose24 extends CommandOpMode {
    ...
    static final double PRELOAD_HOOD       = 0.3;   // flatter — preload shoots from closer / on the move
    ...
    static final double GATE_APPROACH_X  = 15.0;
    ...
    // ── Timing ──────────────────────────────────────────────────────────────
    static final long BURST_MS     = 300;   // feed time — was 750; matched to Zayan's ~500
    ...
    public void initialize() {
        ...
        // ── Command schedule ──────────────────────────────────────────────
        negabot.schedule(
                new InstantCommand(() -> negabot.shooter.setHoodPosition(PRELOAD_HOOD)),   // preload hood; reset after

                new SequentialCommandGroup(

                        // ── 1: Preload — (40,90) @ 2950 RPM ─────────────────────
                        new InstantCommand(() -> negabot.shooter.setMagazineCover(COVER_OPEN)),
                        new ParallelDeadlineGroup(                                     // shoot the preload WHILE driving
                                new FollowPathCommand(f, toShoot0, true),             // drive to the shoot spot (deadline)
                                new RunCommand(this::aimTurretAtGoal, negabot.turret),// keep the turret on the goal
                                new InstantCommand(() -> negabot.intake.setSpeed(INTAKE_ON))  // feed the whole way
                        ),
                        new InstantCommand(() -> {
                            negabot.intake.setSpeed(0);
                            negabot.shooter.setMagazineCover(COVER_CLOSE);
                            negabot.shooter.setHoodPosition(SHOOT_HOOD);   // back to normal hood for the rest
                        }),
...
//                        burst(),
                        ...
                        new InstantCommand(() -> negabot.intake.setSpeed(INTAKE_ON)),
                        new FollowPathCommand(f, toGateFarStraight, true),
                        new WaitCommand(GATE_WAIT_MS),
                        new InstantCommand(() -> negabot.intake.setSpeed(0)),
                        new InstantCommand(() -> negabot.shooter.setMagazineCover(COVER_OPEN)),
                        shotPrep(f, toShootFromGateStraight),
